...
def index():
    ...
    #Initiallize variables for total values
    total_value = cash
    grand_total = cash
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("must provide a positive intiger number of shares")
...
def register():
    ...
    if request.method == "POST":
        ...
        # Ensure password and confirmation mach
        ...
        # Find user id
        current_user_id = db.execute("SELECT * FROM users WHERE username = ?", username)[0]["id"]
